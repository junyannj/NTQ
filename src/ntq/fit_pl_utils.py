...
from . import covariance_utils, config_utils


def log_probability_pl_wrapper(
    theta,
    data_pl,
    co_var_mat_pl,
    X,
    ...
    result = generate_data_utils.log_probability_pl(
        theta,
        data_pl,
        co_var_mat_pl,
        X,
...
def get_maximum_likelihood_estimate(
    ...
    for key in ["EX", "CT", "D"]:
        ...
        for key2 in params_to_fit[key].keys():
            ...
            counter += 1
    print("Maximum log likelihood:", soln_min.fun)
...
def plot_exp_data_with_variance(
    ...
    if fig is None:
        fig, axes = plt.subplots(1, len(temperature_list_pl), figsize=(24, 4.5), dpi=300)
        if len(temperature_list_pl) == 1:
            axes = [axes]
    else:
        axes = axis
    ...
    for i in range(len(temperature_list_pl)):
        ...
        ax.tick_params(axis='x', labelsize=11, direction='in')
        ax.set_xlim(0.8, 1.8)
        ax.set_ylim(0, 1.1)
        ax.set_aspect(10/11)
